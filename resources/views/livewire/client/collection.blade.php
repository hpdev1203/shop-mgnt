            ...
            <div class="w-full md:w-1/3 xl:w-1/4 p-6 flex flex-col">
                <a href="{{route('productdetail')}}">
                    @if (count($product->productDetails) > 0 && $product->productDetails[0] && $product->productDetails[0]->image)
                        ...
                        @endphp
                        <img  class="hover:grow hover:shadow-lg w-full h-80 bg-cover" src="{{ asset('storage/images/products/' . $imageThumbnail) }}" alt="Hình ảnh sản phẩm" class="w-15 h-15 shadow-md">
                    @else
                        <img  class="hover:grow hover:shadow-lg w-full h-80 bg-cover"  src="{{ asset('library/images/image-not-found.jpg') }}" alt="Không có hình ảnh sản phẩm" class="w-15 h-15 shadow-md">
                    @endif
