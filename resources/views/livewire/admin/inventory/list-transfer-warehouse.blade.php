        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th scope="col" class="px-6 py-4 text-xs font-medium text-gray-500 uppercase tracking-wider w-52 text-left">Mã chuyển kho</th>
                    <th scope="col" class="px-6 py-4 text-xs font-medium text-gray-500 uppercase tracking-wider text-left">Tiêu đề</th>
                    <th scope="col" class="px-6 py-4 text-xs font-medium text-gray-500 uppercase tracking-wider w-64 text-left">Kho hàng đi</th>
                    <th scope="col" class="px-6 py-4 text-xs font-medium text-gray-500 uppercase tracking-wider w-64 text-left">Kho hàng đến</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200 text-sm	">
                ...
                <tr>
                    <td class="px-6 py-2 whitespace-nowrap text-center">{{$transfer_warehouses->perPage() * ($transfer_warehouses->currentPage() - 1) + $loop->iteration }}</td>
                    <td class="px-6 py-2 whitespace-nowrap">{{$transfer_warehouse->code}}</td>
                    <td class="px-6 py-2 whitespace-nowrap">{{$transfer_warehouse->name}}</td>
                    <td class="px-6 py-2 whitespace-nowrap">{{$transfer_warehouse->from_warehouse->name}}</td>
                    <td class="px-6 py-2 whitespace-nowrap">{{$transfer_warehouse->to_warehouse->name}}</td>
                    <td class="px-6 py-2 whitespace-nowrap text-center">
                        ...
                        <a data-modal-target="static-modal" data-modal-toggle="static-modal" onclick="parseDataDelete('{{$transfer_warehouse->id}}', '{{$transfer_warehouse->name}}')" class="cursor-pointer inline-flex items-center text-red-600 hover:text-red-900">
