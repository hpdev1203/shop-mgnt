<div class="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
    <div class="px-4 py-6 md:px-6 xl:px-7.5">
        <div class="flex justify-between items-center">
            <h4 class="text-xl font-bold text-black dark:text-white inline">DANH SÁCH PHƯƠNG THỨC THANH TOÁN</h4>
            <a href="{{route('admin.payment-methods.add')}}" class="inline-flex items-center px-4 py-2 bg-blue-500 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-600 active:bg-blue-700 focus:outline-none focus:border-blue-900 focus:ring ring-blue-300 disabled:opacity-25 transition ease-in-out duration-150">
                Thêm mới
            </a>
        </div>
    </div>
            ...
            <thead class="bg-gray-200">
                <tr>
                    <th scope="col" class="px-2 py-4 text-sm font-medium text-gray-700 uppercase tracking-wider w-10 text-center">
                        <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" onclick="checkDeleteMultiple('cbx_delete_payment_method');" class="cursor-pointer">
                    ...
                    <th scope="col" class="px-2 py-4 text-sm font-medium text-gray-700 uppercase tracking-wider text-center">Trạng Thái</th>
                    <th scope="col" class="px-2 py-4 text-sm font-medium text-gray-700 uppercase tracking-wider text-center">Mặc Định</th>
                ...
                @foreach ($payment_methods as $index => $payment_method)
                    <tr>
                        ...
                        <td class="px-2 py-2 whitespace-nowrap" title="{{$payment_method->description}}">
                            @if (strlen($payment_method->description) > 50)
                                {{ substr($payment_method->description, 0, 50) }}...
                            @else
                        ...
                        <td class="px-2 py-2 whitespace-nowrap text-center">
                            <a href="{{route('admin.payment-methods.edit', $payment_method->id)}}" class="inline-flex items-center mr-2 text-indigo-600 hover:text-indigo-900">
                            ...
                            <a data-modal-target="popup-delete-item" data-modal-toggle="popup-delete-item" onclick="parseDataDelete('{{$payment_method->id}}', '{{$payment_method->name}}')" class="inline-flex items-center text-red-600 hover:text-red-900 cursor-pointer">
    ...
    <div class="px-4 py-6 md:px-6 xl:px-7.5">
        {{$payment_methods->links('livewire.custom-pagination')}}
    </div>
