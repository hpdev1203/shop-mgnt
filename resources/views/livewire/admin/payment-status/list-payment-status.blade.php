<div class="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
    <div class="px-4 py-6 md:px-6 xl:px-7.5">
        <div class="flex justify-between items-center">
            <h4 class="text-xl font-bold text-black dark:text-white inline">DANH SÁCH TRẠNG THÁI THANH TOÁN</h4>
            <a href="{{route('admin.payment-status.add')}}" class="inline-flex items-center px-4 py-2 bg-blue-500 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-600 active:bg-blue-700 focus:outline-none focus:border-blue-900 focus:ring ring-blue-300 disabled:opacity-25 transition ease-in-out duration-150">
                Thêm mới
            </a>
        </div>
    </div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-200">
                <tr>
                    ...
                    <th scope="col" class="px-2 py-4 text-sm font-medium text-gray-700 uppercase tracking-wider w-52 text-left">Mã trạng thái</th>
                    <th scope="col" class="px-2 py-4 text-sm font-medium text-gray-700 uppercase tracking-wider text-left">Tiêu đề</th>
                    <th scope="col" class="px-2 py-4 text-sm font-medium text-gray-700 uppercase tracking-wider w-40 text-left">Mặc định</th>
                    <th scope="col" class="px-2 py-4 text-sm font-medium text-gray-700 uppercase tracking-wider w-40 text-left">Trạng thái</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200 text-sm	">
                @foreach ($payment_status as $index => $payment_stt)
                <tr>
                    <td class="px-2 py-2 whitespace-nowrap text-center">
                        <input type="checkbox" name="cbx_delete" class="cursor-pointer w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" wire:model="selected_index.{{$index}}" value="{{$payment_stt->id}}">
                    </td>
                    <td class="px-2 py-2 whitespace-nowrap text-center">{{$payment_status->perPage() * ($payment_status->currentPage() - 1) + $loop->iteration }}</td>
                    <td class="px-2 py-2 whitespace-nowrap">{{$payment_stt->code}}</td>
                    <td class="px-2 py-2 whitespace-nowrap">{{$payment_stt->name}}</td>
                    <td class="px-2 py-2 whitespace-nowrap">
                        @if ($payment_stt->is_default)
                    ...
                    <td class="px-2 py-2 whitespace-nowrap text-center">
                        <a href="{{route('admin.payment-status.edit', $payment_stt->id)}}" class="inline-flex items-center mr-2 text-indigo-600 hover:text-indigo-900">
                        ...
                        <a data-modal-target="popup-delete-item" data-modal-toggle="popup-delete-item" onclick="parseDataDelete('{{$payment_stt->id}}', '{{$payment_stt->name}}')" class="cursor-pointer inline-flex items-center text-red-600 hover:text-red-900">
    ...
    <div class="px-4 py-6 md:px-6 xl:px-7.5">
        {{$payment_status->links('admin.layouts.custom-pagination')}}
    </div>
