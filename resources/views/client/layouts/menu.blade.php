        ...
        <div class="hidden md:flex md:items-center md:w-auto w-full order-3 md:order-1" id="menu">
            <nav>
                <ul class="md:flex items-center justify-between text-base text-gray-700 pt-4 md:pt-0">
                    <li><a class="inline-block no-underline hover:text-black hover:underline py-2 px-4" href="{{route('product');}}">Sản phẩm</a></li>
                ...
                <div class="relative" x-data="{ dropdownOpen: false }" @click.outside="dropdownOpen = false">
                    <a class="flex items-center gap-4" href="#" @click.prevent="dropdownOpen = ! dropdownOpen">
                        <span class="hidden text-right lg:block">
                            <span class="block text-sm font-medium text-black dark:text-white">Xin chào {{ $user->name }}</span>
                        </span>
                        <span class="h-12 w-12 rounded-full">
                            @if($user->avatar_user)
                                <img class="rounded-full" src="{{ asset('storage/images/users/' . $user->avatar_user) }}" alt="{{ $user->name }}" />
                            @else
                    ...
                    <div x-show="dropdownOpen" class="absolute right-0 mt-4 z-999 flex w-62.5 flex-col rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
                        ...
                        <button class="flex items-center gap-3.5 px-6 py-3 text-sm font-medium duration-300 ease-in-out hover:text-primary lg:text-base">
                            <a  href="{{route('logout')}}" class="flex items-center gap-3.5 text-sm font-medium duration-300 ease-in-out hover:text-primary lg:text-base">
